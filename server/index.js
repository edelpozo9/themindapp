...
io.on("connection", (socket) => {
  ...
  socket.on("crearPartida", (nombre, numJugadores) => {
    if (partidas[nombre]) {
      ...
      console.log(`Error: Intento de crear una partida ya existente: ${nombre}`);
    } else {
      partidas[nombre] = {
        nombre,
        numJugadores,
  ...
  socket.on("unirsePartida", (nombrePartida) => {
    ...
    if (partida) {
      ...
      if (Object.keys(partida.jugadores).length >= partida.numJugadores) {
        ...
        console.log(`Jugador ${jugadorID} intentó unirse a una partida llena: ${nombrePartida}`);
      } else {
        ...
        const jugadores = Object.keys(partida.jugadores).map(id => `Jugador ${id}`);
...
socket.on("disconnect", (reason) => {
  ...
  for (const partidaNombre in partidas) {
      ...
      if (partida.jugadores[socket.id]) {
          ...
          console.log(`Jugador ${socket.id} eliminado de la partida ${partidaNombre}`);
          ...
          if (Object.keys(partida.jugadores).length === 0) {
              ...
              console.log(`Partida ${partidaNombre} eliminada porque no quedan jugadores.`);
          }
...
function dejarPartida(socket, nombrePartida) {
  ...
  if (partida && partida.jugadores[socket.id]) {
    ...
    if (Object.keys(partida.jugadores).length === 0) {
      ...
      console.log(`La partida ${nombrePartida} ha sido eliminada porque no tiene jugadores.`);
    }
  }
}
